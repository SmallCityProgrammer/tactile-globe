...
import textura as T
from estilo import MATA, ESC_MATA, sobre, simples, MM, _mm, dd, grava_svg, CHAO
...
# A DENSIDADE TAMBEM VAI PARA O CHAO, junto com a copa.
# Se a copa cresce com o zoom e a contagem continua por area de TELA, o zoom
# fechado poe o mesmo numero de arvores num pedaco muito menor de terreno, cada
# uma agora enorme: o bosque vira um tapete verde sem uma arvore dentro. As duas
# medidas tem que morar na mesma unidade, e a unidade certa e a do mundo.
#
# 40 arvores por 10 000 m2 (um hectare) e mata de bocage normando: copa de 6 a
# 12 m encostando na vizinha sem fechar dossel.
DENS        = 40                  # arvores por DENS_AREA
DENS_AREA   = 10000.0             # m2 de chao
SEMENTE     = 1941

ENGORDA     = 7.0                 # metros que as arvores transbordam o poligono
...
def sorteio(sub, por=DENS, area=DENS_AREA, semente=SEMENTE, clip=False, por_feicao=True):
    ...
    DensityBasedCount + densityArea em METROS DE CHAO prende a densidade ao
    terreno, que e onde a arvore existe. Com AbsoluteCount cada poligono ganharia
    o mesmo numero de arvores, e um bosque de 20 m ficaria tao cheio quanto um de
    600 m.
    ...
    sl.setDensityArea(area); sl.setDensityAreaUnit(CHAO)
...
def gerador(expr, sub_fill, unidade=None):
    unidade = CHAO if unidade is None else unidade
    """
...
# A COPA EM METROS DE CHAO, como a rua — veja CHAO no estilo.py.
# Uma arvore adulta de bocage tem de seis a doze metros de copa. Em milimetro de
# tela ela sai com 6 px em qualquer placa: no quadro largo parece mata, e no
# quadro fechado, onde a casa tem 300 px, vira poeira verde. O ganho do zoom ia
# para tudo menos para a arvore.
TAM_PNG_MIN, TAM_PNG_MAX = 6.0, 12.0  # metros; a copa em PNG carrega a propria sombra,
...
def copa_png(pasta, desvio=0):
    ...
    m.setPath(caminhos[0]); m.setSize(TAM_PNG_MIN); m.setSizeUnit(CHAO)
